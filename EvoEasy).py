...
#—————————————————————————————————————————————————————————————————————————————————
# Idea: sqlmerr
# meta developer: @Rodik_Godmodules
# Thanks: 
#—————————————————————————————————————————————————————————————————————————————————
...
@loader.tds
class EvoEasy(loader.Module):
    """Модуль для управления аккаунтом в MineEVO\nУстанавливайте модуль исключительно из @Rodik_Godmodules"""
    strings = {
        "name" : "EvoEasy",
    ...
    async def client_ready(self):
        ...
        self._backup_channel, _ = await utils.asset_channel(
            self._client,
            "EVOQasst",
            "Группа для работы модуля EVO от @Rodik_Godmodules\nНе добавляйте сюда других людей или ботов!",
            silent=True,
            archive=True,
            _folder="hikka",
        )
        ...
        self._drill_channel, _ = await utils.asset_channel(
            self._client,
            "EVO_Drill",
            "Группа для работы auto_collect_drill в модуле EVO от @Rodik_Godmodules\nНе добавляйте сюда других людей или ботов!",
            silent=True,
            archive=True,
            _folder="hikka",
        )
    ...
    async def donate_author(self, call: InlineCall):
        await self.client.send_message('@mine_evo_bot','Дать Rodik_God к 1')
